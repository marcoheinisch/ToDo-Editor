...
class SyntaxHighlighting():
    ...
    def __init__(self, parent, text_widget, initial_content):
        ...
        self.patternactions = [
            ...
            (r"(\d\d\:\d\d)", 'Token.Literal.Number.Integer'), 
            (r"(==.*)", 'Token.Keyword'),  
            (r"(x.*)", 'Token.Name.Decorator'), 
            (r"(\d\d\.\d\d\.(\d\d(\d\d)?)?)", 'Token.Name.Builtin'), 
            (r"(<.*)", 'Token.Keyword'),
    ...
    def default_highlight(self):
        ...
        if (self.previousContent != content):
            print(location)
            for pa in self.patternactions:
                self.text.highlight_pattern(pa[0],pa[1],regexp=True)
